...
function logger(opt) {
    var winston = require("winston"), options = opt || Object.create(null), my = {
        filename: require("path").resolve(String(options.filename)) || !1,
    ...
    optional.filename = my.filename, optional.logger = String(optional.logger || "logger-request"), 
    optional.level = String(optional.level || "info"), optional.timestamp = optional.timestamp || !0, 
    optional.maxsize = Number(optional.maxsize) || 8388608, optional.maxFiles = Number(optional.maxFiles) || null, 
    optional.json = optional.json === !1 ? !1 : !0, optional.raw = optional.raw === !1 ? !1 : !0, 
    my.logger = optional.logger;
    var log = new winston.Logger();
    my.filename && log.add(winston.transports.File, optional), Boolean(options.console) && log.add(winston.transports.Console, optional);
    for (var i = 0; i < my.transports.length; i++) log.add(my.transports[i], optional);
    if (log = log[optional.level], Boolean(options.standalone)) return log;
    ...
    var io = info({
        ...
        callback: "function" == typeof options.callback ? options.callback : !1
    });
